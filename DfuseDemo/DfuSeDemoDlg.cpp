...
#define STVendorID		  0x0483
#define ST_PAGE			  0x008C 
#define VENDOR_USAGE_PAGE 0xFF00
#define USAGE_DETACH	  0x0055
...
#define COLOR_UPLOAD_FG		(RGB(0x00,0x80,0xFF))
#define COLOR_UPLOAD_BG		(RGB_BK)
#define COLOR_UPGRADE_FG	(RGB(0x00,0x80,0xFF))
...
BEGIN_MESSAGE_MAP(CDfuSeDemoDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_WM_DEVICECHANGE()
	ON_BN_CLICKED(IDCANCEL, &CDfuSeDemoDlg::OnBnClickedCancel)
END_MESSAGE_MAP()
...
BOOL CDfuSeDemoDlg::OnInitDialog()
{
	...
	CString ExePath = path_buffer;
	CString IniPath = ExePath + "Config.ini";
	//read Sting and Version from ini 
	char iniStrVer[100];
	memset(iniStrVer, 0, 100);
	GetPrivateProfileString ("Setting", "StringVersion", "", iniStrVer, 100, IniPath); 
	CString sStrVer=iniStrVer;
	SetWindowTextA(sStrVer);

	SetIcon(m_hIcon, TRUE);			// Set big icon
	//SetIcon(m_hIcon, FALSE);		// Set small icon

	if (InstallBootloaderDriver())
	{
		LoadFWFile();
	}
	...
	GetDlgItem(IDC_STATIC_FW)->SetFont(f);
...
CString getOsVersion(char* OSVersion)
{
	...
	//if (!strcmp(swVersion,"6.1"))   strcpy( OSVersion,"win7");     // win7 
	//else if (!strcmp(swVersion,"6.2"))   strcpy( OSVersion,"win8");      // win8 
	//else if (!strcmp(swVersion,"6.3*"))   strcpy( OSVersion,"win8.1");     // win8.1 
	//else if (!strcmp(swVersion,"10.0*"))   strcpy( OSVersion,"win10");      // win10 

	// 获取版本号,此方法不依赖于上面例子中的 manifest 文件，也不受兼容性的影响。需要引入下面头和库
	//#include <windows.h>
	//#include <lm.h>
	//#pragma comment(lib, "netapi32.lib")
	...
	if (netStatus == NERR_Success) 
	{
		...
		char swVersion[10]={0};    
		sprintf(swVersion,"%d.%d",dwMajVer,dwMinVer);
		// dwVesion=atoi(swVersion);  
...
CString GetBootloaderDriverPath()
{
	CString sDriverPath="";

	char szOSVersion[40]={0};
	CString OSVersion = getOsVersion(szOSVersion);  //获得操作系统名称
	int lzBit=GetSystemBits(); // 获取操作系统位数
	if (OSVersion == "win7")
	{
		if (lzBit == 32) 
			sDriverPath="Driver\\Win7\\x86\\dpinst_x86.exe";
		else 
			sDriverPath="Driver\\Win7\\x64\\dpinst_amd64.exe";
	}
	else if (OSVersion == "win8")
	{
		if (lzBit == 32) 
			sDriverPath="Driver\\Win8\\x86\\dpinst_x86.exe";
		else 
			sDriverPath="Driver\\Win8\\x64\\dpinst_amd64.exe";
	}
	else if (OSVersion == "win8.1"||OSVersion == "win10")
	{
		if (lzBit == 32) 
			sDriverPath="Driver\\Win8.1\\x86\\dpinst_x86.exe";
		else 
			sDriverPath="Driver\\Win8.1\\x64\\dpinst_amd64.exe";
	}

	return sDriverPath;
}

bool CDfuSeDemoDlg::InstallBootloaderDriver()
{
	char path_buffer[_MAX_PATH];
	char drive[_MAX_DRIVE];
	char dir[_MAX_DIR];
	...
	_makepath( path_buffer, drive, dir, NULL,NULL );
	CString ExePath = path_buffer;
	//CString IniPath = ExePath + "DfuSe_Demo_V3.0_Setup.exe";
	//CString IniPath = ExePath + "Config.ini";
	////read Bootloader driver name from ini 
	//char iniBootloader[50];
	//memset(iniBootloader, 0, 50);
	//GetPrivateProfileString ("Setting", "Bootloader", "", iniBootloader, 50, IniPath); 
	
    CString sFWFile=ExePath + GetBootloaderDriverPath();
...
void CDfuSeDemoDlg::LoadFWFile()
{
	...
	char iniFW[100];
	memset(iniFW, 0, 100);
	GetPrivateProfileString ("Setting", "FW", "", iniFW, 100, IniPath); 
	m_DownFileName=ExePath + iniFW;
	...
	if (STDFUFILES_OpenExistingDFUFile((LPSTR)(LPCSTR)m_DownFileName, &hFile, &Vid, &Pid, &Bcd, &NbTargets)==STDFUFILES_NOERROR)
	{
		...
		m_DownFile.SetWindowText(iniFW);
...
void CDfuSeDemoDlg::OnButtonupgrade() 
{
	...
	if (DevId.IsEmpty())
	{
		//if (AfxMessageBox("Your device was plugged in DFU mode. \nMake sure this file is correct for this device.\n", MB_YESNO)!=IDYES)
		//	return;
	}
	else
	{
			...
			case OPERATION_UPLOAD:
				{
				  ...
				  Tmp.Format("Uploading %i%%...", Context.Percent);
...
void CDfuSeDemoDlg::OnBnClickedCancel()
{
